...
export const clientsService = {
  ...
  getAll: () =>
    api.get<ApiResponse<Client[]>>('/bop/clients').then((response) => response.data ?? []),

  getById: (id: string) =>
    api.get<ApiResponse<Client>>(`/bop/clients/${id}`).then((response) => response.data),

  updateStatus: (id: string, status: string) =>
    api.put<ApiResponse>(`/bop/clients/${id}/status`, { status }),

  getReport: (id: string) =>
    api.get<ApiResponse<ClientReport>>(`/bop/clients/${id}/report`).then((response) => response.data ?? response),

  getVouchers: (id: string) =>
    api.get<ApiResponse<Voucher[]>>(`/bop/vouchers/client/${id}`).then((response) => response.data ?? []),

  getPackages: (id: string) =>
    api.get<ApiResponse<Package[]>>(`/bop/clients/${id}/packages`).then((response) => response.data ?? []),
...
export const advertsService = {
  ...
  updateStatus: (id: string, status: 'active' | 'inactive' | 'pending') =>
    api.put<ApiResponse>(`/bop/adverts/${id}/status`, { status }),
...
export const bopDevicesService = {
  ...
  getByVoucher: (voucherId: string) =>
    api.get<ApiResponse<BopDevice[]>>(`/bop/devices/voucher/${voucherId}`).then((response) => response.data ?? []),

  getById: (id: string) =>
    api.get<ApiResponse<BopDevice>>(`/bop/devices/${id}`).then((response) => response.data ?? response),
